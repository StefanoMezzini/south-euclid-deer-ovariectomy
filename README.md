The code, data, and models found here were used for the analyses in:
...
- `location-long`: numeric; longitude of the fix.
- `location-lat`: numeric; latitude of the fix.
- `behavioural-classification`: character; behavioral classification as one of `STOP`, `START`, `ON_THE_MOVE`, `PERIODIC`, or `NA`.
- `comments`: character; addresses near the fix.
...
- `long`: numeric; longitude of the fix.
- `lat`: numeric; latitude of the fix.
